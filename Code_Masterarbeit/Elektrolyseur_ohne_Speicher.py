...
investment_cost_ely = 600  # Beispiel-Investitionskosten in Euro/kW
...
slope_h2, offset_h2 = solph.components._offset_converter.slope_offset_from_nonconvex_input(P_in_max, P_in_min, eta_at_max=0.60, eta_at_min=0.7)

slope_heat, offset_heat = solph.components._offset_converter.slope_offset_from_nonconvex_input(P_in_max, P_in_min, eta_at_max=0.35, eta_at_min=0.25)


#Daten Teillastverahlten
eta_h2_min_2 = 0 #efficiency at P_in_min
eta_h2_max_2 = 0.7 #efficiency at P_in_max
P_in_min_2 = 0
P_in_max_2 = 2
...
slope_heat_2, offset_heat_2 = solph.components._offset_converter.slope_offset_from_nonconvex_input(P_in_max_2, P_in_min_2, eta_at_max=0.25, eta_at_min=1)
...
def find_min_lcoh2(c_h2_virtual):
    ...
    menge_sauerstoff = sum(results[("electrolyzer market 1", "oxygen bus")]["sequences"]["flow"].values[:-1] +
                           results[("electrolyzer market 2", "oxygen bus")]["sequences"]["flow"].values[:-1])
    ...
    einnahmen_sauerstoff = sum(results[("electrolyzer market 1", "oxygen bus")]["sequences"]["flow"].values[:-1] +
                               results[("electrolyzer market 2", "oxygen bus")]["sequences"]["flow"].values[
                               :-1]) * c_oxygen
...
for c_h2_virtual in range(2000000,2000001):
    lcoh2, df, df2 = find_min_lcoh2(c_h2_virtual)
