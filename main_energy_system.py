'''
General Descirption:
Model of Electrolyzer which produces hydrogen, heat and oxygen
Electrical and chemical storage is added
Participates at control reserve market, if capacity is available
...
num_tsteps = 500 #Anzahl der Zeitschritte
...
#erstellen einer zufälligen Nachfragezeitreihe
random.seed(42)
...
for n in range(num_tsteps):
    ...
    datetime_index = solph.create_time_index(number=number_tsteps - n, start=start_time)  # number_tsteps
    ...
    es2.add(b_el, b_h2, b_heat, b_o2, b_el_neg_affr_virt, b_h2_neg_affr_virt, b_el_pos_affr_virt, b_h2_pos_affr_virt,
            source_el, sink_h2_demand, sink_heat, sink_o2, source_el_neg_affr_virt, sink_h2_neg_affr_virt,
            source_el_pos_affr_virt, sink_h2_pos_affr_virt,
    ...
    def min_storage_capa_safety(m, t):
        if hydrogen_storage_capacity - hydrogen_storage_initial_storage_level * hydrogen_storage_capacity <= 10:
            expr = (om.flow[electrolyzer1_1, b_h2, t] + om.flow[electrolyzer1_2, b_h2, t]) * (
                        om.flow[electrolyzer2_1, b_h2_neg_affr_virt, t] + om.flow[
                    electrolyzer2_2, b_h2_neg_affr_virt, t]) == 0
            return expr
        else:
            return po.Constraint.Skip
